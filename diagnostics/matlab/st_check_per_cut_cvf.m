...
executionCUTPath = string(field_text(manifest, {'ExecutionCUTPath'}, ...
    field_text(target, {'StandaloneCUTPath'}, '')));
displayRationale = "none";
...
try
    ...
    contentRules = startsWith(selectedPaths, executionCUTPath + "/");
    boundaryRules = ~contentRules;
    ...
    contentPolicy = all(selectorTypes(contentRules) == ...
        ...
        all(ruleRationales(contentRules) == displayRationale);
    boundaryPolicy = all(ismember(selectorTypes(boundaryRules), ...
        ...
        all(ruleRationales(boundaryRules) == displayRationale);
