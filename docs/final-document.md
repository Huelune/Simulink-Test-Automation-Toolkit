...
커버리지를 읽기 전에 모델을 엽니다. 커버리지 필터를 붙이고 커버리지를 읽는 동안
Simulink Coverage가 그 모델과 Harness를 미저장(Dirty)으로 표시하는데, 저장된
내용이 바뀐 것은 아닙니다. 결과 정리는 모델을 절대 저장하지 않습니다.

그래서 결과 정리는 이렇게 뒷정리합니다.

- 자기가 연 모델은 저장하지 않고 닫습니다.
- 시작할 때 이미 열려 있고 깨끗했던 모델은, 정리 중 올라간 Dirty 표시를 다시
  내립니다. 로그에 `Cleared the Dirty flag raised while collecting`이 남습니다.
- 시작할 때 이미 미저장이던 모델은 건드리지 않습니다. 그 변경은 사용자의 것입니다.

그러므로 "결과 정리를 방금 돌렸는데 최종 문서가 미저장이라고 거절한다"가 여전히
나오면, 그 Harness에 결과 정리 전부터 손으로 고친 변경이 있었다는 뜻입니다. 오류
메시지에 어느 모델인지 적혀 있으니 열어 보고 저장하거나 닫은 뒤 다시 부르십시오.
