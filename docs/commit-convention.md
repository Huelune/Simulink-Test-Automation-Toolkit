...
### CHANGELOG는 해당 커밋에 포함합니다

`CHANGELOG.md` 항목은 그 변경을 만든 커밋에 함께 넣습니다. 별도 `docs` 커밋으로
빼면 기능 커밋만 되돌렸을 때 CHANGELOG에 없는 기능이 적힌 채로 남습니다.

같은 이유로, 변경과 분리하면 이력이 어긋나는 다른 파일도 함께 넣습니다.
