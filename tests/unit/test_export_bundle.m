...
verifyTrue(testCase, isfile(fullfile(root, 'resources', ...
    'export_bundle', 'README.review.ko.md')));
end
...
function testReviewExportUsesFastNonReproducibleProfile(testCase)
root = st_project_root();
...
verifyTrue(testCase, contains(wrapperText, ...
    "fullfile(cfg.ExportRootDir, 'review')"));
verifyTrue(testCase, contains(exportText, ...
    'Model dependency analysis: SKIP'));
verifyTrue(testCase, contains(exportText, ...
    'Harness and SLDV input collection: SKIP'));
verifyTrue(testCase, contains(exportText, ...
