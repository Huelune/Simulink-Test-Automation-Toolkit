...
function testRunnerClosesLoadedHarnessForCoverageView(testCase)
root = st_project_root();
source = fileread(fullfile(root, 'src', 'execution', ...
    'st_run_tests_per_cut.m'));

verifyNotEmpty(testCase, regexp(source, ...
    'close_harness_for_coverage_view\(row, cfg\)', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    'HARNESS_CLOSE_FOR_COVERAGE', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    'bdIsLoaded\(harnessName\)', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    'sltest\.harness\.close\(ownerPath, harnessName\)', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    'Coverage-view Harness close start', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    'Coverage-view Harness close complete', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    'Coverage-view Harness close failed', 'once'));
end
