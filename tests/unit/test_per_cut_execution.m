...
verifyNotEmpty(testCase, regexp(source, ...
    'ResultFilterMode'', ''DURING_RUN''', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    'managedApplication = "RESULT_POST_RUN"', 'once'));
verifyEmpty(testCase, regexp(source, ...
...
verifyNotEmpty(testCase, regexp(source, ...
    '''ResultFilterMode'', resultFilterMode', 'once'));
verifyEmpty(testCase, regexp(source, ...
    'applied\.cvf', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    'st_apply_result_coverage_filters', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    'POST_RUN_REQUIRED', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    "addParameter\(p, 'RunRootDirectory', ''", 'once'));
verifyEmpty(testCase, regexp(source, ...
    'verify_result_filter_roundtrip', 'once'));
...
filterGenerationPositions = regexp(source, ...
    'generate_filter\(row, filterDirectory, cfg, logPath, i,');
verifyGreaterThanOrEqual(testCase, numel(filterGenerationPositions), 2);
verifyTrue(testCase, runPosition < filterGenerationPositions(2));
...
verifyNotEmpty(testCase, regexp(resultFilterSource, ...
    'cvd\.filter\s*=\s*propertyValue', 'once'));
verifyNotEmpty(testCase, regexp(resultFilterSource, ...
...
verifyNotEmpty(testCase, regexp(source, ...
    'ResultFilterStatus', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    'ResultFilterAttachCount', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    '''FILTER_RESTORE''', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    '''MODEL_CLEANUP''', 'once'));
...
verifyNotEmpty(testCase, regexp(source, ...
    'Status\(i\) = "EXCEPT"[\s\S]*?TARGET_EXCEPT', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    '\["FAIL","EXCEPT","WARN"\]', 'once'));
end
...
verifyNotEmpty(testCase, regexp(source, ...
    'cvsave\(basePath, cvd\)', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    'coverage_integrity_snapshot\(resultObj\)', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    'ResultCoverageChangedDuringExport', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    "st_enter_writable_coverage_directory[\s\S]*?'CVSAVE'", 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    "st_enter_writable_coverage_directory[\s\S]*?'CVHTML'", 'once'));
verifyEmpty(testCase, regexp(source, 'Id = zeros\(n,1\)', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    'Checksum = strings\(n,1\)', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
