...
items = [ ...
    decision_json('Switch', 'Switch', 'Top/CUT/Switch', 'T/F', 'u2 > 0'); ...
    decision_json('Switch', 'Switch2', 'Top/CUT/Switch2', 'T/F', 'u2 ~= 0'); ...
    decision_json('If', 'If (a==1)', 'Top/CUT/If', 'T/F', 'a==1'); ...
    decision_json('If', 'elseIF(a!=1)', 'Top/CUT/ElseIf', 'T/F', 'a~=1'); ...
    decision_json('If', 'else', 'Top/CUT/Else', 'T/F', 'true')];
raw = "[" + newline + strjoin(items, "," + newline) + newline + "]";
...
expected = strjoin(["Switch"; "D1 [T/F]Switch (u2 > 0)"; ...
    "Switch2"; "D2 [T/F]Switch (u2 ~= 0)"; ...
    "If (a==1)"; "D3 [T/F]IF (a==1)"; ...
    "elseIF(a!=1)"; "D4 [T/F]IF (a~=1)"; ...
    "else"; "D5 [T/F]IF (true)"], newline);
...
verifyEqual(testCase, decisionDetails.Outcome(1:5), repmat("T/F", 5, 1));
verifyEqual(testCase, decisionDetails.Expression(1:5), ...
    ["u2 > 0"; "u2 ~= 0"; "a==1"; "a~=1"; "true"]);
...
function text = decision_json(blockType, name, path, outcome, expression)
item = struct('BlockType', blockType, 'Name', name, 'Path', path, ...
    'Outcome', outcome, 'Expression', expression, ...
    'ExpressionStatus', 'OK', 'Message', '');
text = string(jsonencode(item));
end
...
function testMainSheetUsesTFWhileDetailSheetKeepsSpecificOutcome(testCase)
% The main cell marks every branch the same way so the sheet reads
% uniformly. The specific branch kind stays in DecisionBlockDetails.
folder = tempname;
mkdir(folder);
cleanup = onCleanup(@() rmdir(folder, 's')); %#ok<NASGU>
path = fullfile(folder, 'decision_outcomes.xlsx');
items = [ ...
    decision_json('If', 'If1', 'Top/CUT/If', 'T/F', 'a==1'); ...
    decision_json('MinMax', 'Mx', 'Top/CUT/Mx', 'SELECT', 'max; Inputs=3'); ...
    decision_json('SwitchCase', 'Sc', 'Top/CUT/Sc', 'CASE', 'u1 in {1,[7,9]}'); ...
    decision_json('Saturate', 'Sat', 'Top/CUT/Sat', 'LIMIT', ...
        'UpperLimit=1; LowerLimit=-1'); ...
    decision_json('Lookup_n-D', 'LUT', 'Top/CUT/LUT', 'INTERVAL', ...
        'NumberOfTableDimensions=1'); ...
    decision_json('Relay', 'Rly', 'Top/CUT/Rly', 'ON/OFF', ...
        'OnSwitchValue=0.5; OffSwitchValue=-0.5')];
raw = "[" + newline + strjoin(items, "," + newline) + newline + "]";
specification = table("Case1", "Top/CUT", raw, "", 'VariableNames', ...
    {'테스트 케이스명','CUTPath','DecisionBlocks','비고'});
assessmentDetails = table(strings(0,1), 'VariableNames', {'Message'});
[written, ~] = st_write_specification_workbook( ...
    specification, assessmentDetails, path);
expected = strjoin(["If1"; "D1 [T/F]IF (a==1)"; ...
    "Mx"; "D2 [T/F]MinMax (max; Inputs=3)"; ...
    "Sc"; "D3 [T/F]SwitchCase"; ...
    "Sat"; "D4 [T/F]Saturate (UpperLimit=1; LowerLimit=-1)"; ...
    "LUT"; "D5 [T/F]Lookup_n-D (NumberOfTableDimensions=1)"; ...
    "Rly"; "D6 [T/F]Relay (OnSwitchValue=0.5; OffSwitchValue=-0.5)"], newline);
verifyEqual(testCase, written.DecisionBlocks(1), expected);
readback = readtable(path, 'Sheet', 'TestSpecification', 'TextType', 'string', ...
    'VariableNamingRule', 'preserve');
verifyEqual(testCase, readback.DecisionBlocks(1), expected);
decisionDetails = readtable(path, 'Sheet', 'DecisionBlockDetails', 'TextType', 'string');
verifyEqual(testCase, decisionDetails.Decision(1:6), compose("D%d", (1:6).'));
verifyEqual(testCase, decisionDetails.Outcome(1:6), ...
    ["T/F"; "SELECT"; "CASE"; "LIMIT"; "INTERVAL"; "ON/OFF"]);
verifyEqual(testCase, decisionDetails.BlockType(1:6), ...
    ["If"; "MinMax"; "SwitchCase"; "Saturate"; "Lookup_n-D"; "Relay"]);
% SwitchCase hides its condition in the main cell but must keep it here.
verifyEqual(testCase, decisionDetails.Expression(3), "u1 in {1,[7,9]}");
verifyTrue(testCase, all(decisionDetails.ReadStatus == "OK"));
end

function testOneBlockWithSeveralBranchesPrintsItsNameOnce(testCase)
% An If block with two elseif conditions occupies three D numbers, and the
% block name heads the group instead of repeating above every branch.
folder = tempname;
mkdir(folder);
cleanup = onCleanup(@() rmdir(folder, 's')); %#ok<NASGU>
path = fullfile(folder, 'decision_branches.xlsx');
items = [ ...
    decision_json('If', 'Mode If', 'Top/CUT/If', 'T/F', 'u1 == 0'); ...
    decision_json('If', 'Mode If', 'Top/CUT/If', 'T/F', 'elseif u2 > 1'); ...
    decision_json('If', 'Mode If', 'Top/CUT/If', 'T/F', 'elseif u3 < 2'); ...
    decision_json('Switch', 'Sw', 'Top/CUT/Sw', 'T/F', 'u2 > 0')];
raw = "[" + newline + strjoin(items, "," + newline) + newline + "]";
specification = table("Case1", "Top/CUT", raw, "", 'VariableNames', ...
    {'테스트 케이스명','CUTPath','DecisionBlocks','비고'});
assessmentDetails = table(strings(0,1), 'VariableNames', {'Message'});
[written, ~] = st_write_specification_workbook( ...
    specification, assessmentDetails, path);
expected = strjoin(["Mode If"; "D1 [T/F]IF (u1 == 0)"; ...
    "D2 [T/F]IF (elseif u2 > 1)"; ...
    "D3 [T/F]IF (elseif u3 < 2)"; ...
    "Sw"; "D4 [T/F]Switch (u2 > 0)"], newline);
verifyEqual(testCase, written.DecisionBlocks(1), expected);
decisionDetails = readtable(path, 'Sheet', 'DecisionBlockDetails', 'TextType', 'string');
verifyEqual(testCase, decisionDetails.Decision(1:4), compose("D%d", (1:4).'));
verifyEqual(testCase, decisionDetails.Name(1:4), ...
