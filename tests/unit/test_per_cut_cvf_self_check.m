...
verifyNotEmpty(testCase, regexp(source, ...
    'contentRules\s*=\s*startsWith\(selectedPaths, executionCUTPath', ...
    'once'));
verifyNotEmpty(testCase, regexp(source, ...
    'displayRationale\s*=\s*"none"', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
...
verifyNotEmpty(testCase, regexp(source, ...
    'ruleRationales\(contentRules\)\s*==\s*displayRationale', 'once'));
verifyNotEmpty(testCase, regexp(source, ...
    'ruleRationales\(boundaryRules\)\s*==\s*displayRationale', 'once'));
end
