...
- **Enabled / Triggered Subsystem도 분기 블록으로 셉니다.** enable과 trigger
  ...
  - 그 분기는 Subsystem 한 겹 안의 port 블록에 붙어 있어 직계 검색에 걸리지
    ...
    없습니다.
  - **CUT 자신이 조건부인 경우만 셉니다.** 그 분기는 Subsystem 한 겹 안의 port
    블록에 붙어 있어 위에서 깊이 1로 찾으면 안 보이고, 다른 어떤 스캔도 대신
    잡아 주지 않습니다. 자식 서브시스템의 enable은 그 자식의 분기이므로
    제외합니다. 더 깊은 곳도 제외합니다.
  - 평범한 Subsystem은 그대로 빠집니다. 자기 분기가 없기 때문입니다.
    커버리지 쪽은 objective를 port 블록에 물어 안쪽 합계가 섞이지 않게 합니다.
  - **이 두 종류만은 커버리지 목록이 아니라 모델에서 직접 읽습니다.**
    서브시스템이 조건부인지는 구조적 사실이라 커버리지에 물을 이유가 없습니다.
...
- **standalone 제출물을 팀 제출 트리로 재배치하는 Python 스크립트를 추가했습니다.**
  ...
  `프로젝트/`로 복사합니다. 원본은 그대로 남기고, 세 갈래에 들지 않는 파일과 CUT
  폴더 안의 `scv_images` 폴더는 건너뛴 목록으로만 출력합니다. MATLAB과 툴킷 소스에
  의존하지 않습니다.
...
- **번들 러너가 Simulink 빌드 캐시 설정을 실행 뒤 확실히 되돌립니다.** 실행마다
  ...
  임시 폴더를 지웠습니다. 그 세션의 이후 빌드는 지워진 폴더를 가리키게 됩니다. 이제
  설정 객체 → 폴더 이름 직접 지정 → `reset` 순으로 시도하고, `reset`까지 간 경우에만
  WARN을 남깁니다.

- **최종 문서의 `Description`이 실행에서 실제로 잡힌 분기 블록을 적습니다.**
  지금까지는 저장된 블록 파라미터로 분기 블록을 추정했습니다. 카탈로그에 없는
  종류는 놓치고, 커버리지가 세지 않는 블록은 더 적혔습니다. 실행 결과에 이미
  답이 있으므로 Simulink Coverage에 직접 묻습니다.
  - 결과 정리가 결과 워크북에 `DecisionPoints` 시트를 남깁니다. 커버리지가 그
    CUT에서 Decision objective를 잡은 블록의 경로와 종류입니다. 커버리지 객체와
    로드된 모델이 그 자리에 이미 있을 때 한 번만 훑습니다. `decisioninfo`
    반복 호출은 보고서에서 제일 느린 지점이라 추출할 때마다 되풀이할 일이
    아닙니다.
  - 목록은 **CVF를 거친 뒤의 결과**입니다. 커버리지 필터가 면제한 objective만
    가진 블록은 빠집니다. 필터가 곧 이 CUT이 무엇을 재는가를 정하므로, CUT이
    claim하지 않은 중첩 Subsystem 안쪽이 올라오지 않게 하는 것이 이 규칙입니다.
    필터를 쓰지 않으면 면제되는 것이 없어 아무것도 빠지지 않습니다.
  - **CUT의 직계 블록만 봅니다.** 정적 스캔과 같은 범위이고, 바뀌는 것은 그중
    무엇을 분기로 세느냐뿐입니다. 중첩 Subsystem 안의 분기는 그 Subsystem의
    것입니다. Subsystem과 Model Reference 자체도 뺍니다. 그 수치는 안쪽 합계라서
    남기면 모든 상위 블록이 분기로 보입니다.
  - 블록 경로는 CUT 기준 **상대 경로**로 보관합니다. standalone 번들이 모델
    이름을 바꾸므로 절대 경로는 항상 맞지 않습니다.
  - **커버리지가 없으면 지금까지대로 정적 스캔을 씁니다.** 결과 정리 전에도
    문서가 나옵니다. 어느 쪽에서 왔는지는 `Metadata`의
    `DecisionSourceWorkbooks`와 `DecisionSourceCUTs`에 남습니다.
  - 커버리지에서 왔을 때는 `DecisionBlockScope`를 `ALL`로 넓힙니다. 커버리지가
    인식한 종류를 좁은 카탈로그가 도로 걸러내면 안 됩니다. `'NONE'`은 그대로
    존중합니다.

- **판정을 하나도 못 읽었을 때 증상 대신 원인을 적습니다.** 결과 정리를 하지
  않고 최종 문서를 만들면 모든 행의 확인 사유가 `NO_MATCHING_TEST_RESULT`였는데,
  이것은 증상입니다. 이제 `NOT_COLLECTED`처럼 실행 해석이 찾아 둔 원인을 그대로
  적습니다. 일부라도 읽혔으면 바꾸지 않습니다. 그때의 매칭 실패는 진짜 매칭
  실패이고 수집이 중간에 끊긴 경우와 구별되어야 합니다.
  - `result/per_cut_latest.json`은 실행이 쓰고 수집은 쓰지 않으므로, **정리한
    뒤에 테스트를 다시 돌리면** 포인터가 정리되지 않은 새 실행으로 옮겨가 판정이
    도로 빈 칸이 됩니다. 문서에 적었습니다.
