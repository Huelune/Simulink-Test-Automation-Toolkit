function specification = st_specification_table(rows, verifyCells, maxTimes, decisionBlockLists)
...
if nargin < 4, decisionBlockLists = repmat("[]", size(rows,1), 1); end
decisionBlockLists = string(decisionBlockLists);
if numel(decisionBlockLists) ~= size(rows,1)
    error('simtest:SpecificationRowMismatch', 'Decision block lists must match specification rows.');
end
...
specification = addvars(specification, decisionBlockLists(:), ...
    'After', 'CUTPath', 'NewVariableNames', 'DecisionBlocks');
