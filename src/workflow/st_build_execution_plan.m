...
plan = table(Key, T.No, T.CUTName, T.CUTPath, T.HarnessName, ...
    T.TestCaseName, T.CoverageFilterMode, T.CoverageBoundaryMode, T.CoverageFilterAction, ...
    T.CoverageFilterRationale, Mode, FromStage, ...
    'VariableNames', {'Key','No','CUTName','CUTPath','HarnessName', ...
    'TestCaseName','CoverageFilterMode','CoverageBoundaryMode','CoverageFilterAction', ...
...
coverageFilter = struct('Upstream', signatures.ASSESSMENT, ...
    'Mode', char(row.CoverageFilterMode), ...
    'BoundaryMode', char(row.CoverageBoundaryMode), ...
    'Action', char(row.CoverageFilterAction), ...
    'Rationale', char(row.CoverageFilterRationale), ...
    'ApplicationMode', cfg.CoverageFilterApplicationMode, ...
    'ExecutionMode', cfg.ExecutionMode, ...
    'FilterDirectory', cfg.CoverageFilterDir);
signatures.COVERAGE_FILTER = st_hash_value(coverageFilter);

testManager = struct('Upstream', signatures.COVERAGE_FILTER, ...
