...
% Coverage filters are generated by whoever executes, not by a preparation
% stage. PERSIST still writes them into the Test File, so the filter policy
% stays part of the TEST_MANAGER signature.
testManager = struct('Upstream', signatures.ASSESSMENT, ...
    'CoverageFilterMode', char(row.CoverageFilterMode), ...
    'CoverageBoundaryMode', char(row.CoverageBoundaryMode), ...
    'CoverageFilterAction', char(row.CoverageFilterAction), ...
    'CoverageFilterRationale', char(row.CoverageFilterRationale), ...
    'CoverageFilterDir', cfg.CoverageFilterDir, ...
