...
if executeTests && any(~failedCloneRows) && strcmp(executionMode, 'PER_CUT')
    if strcmpi(cfg.PerCutResultCollection, 'DEFERRED')
        % Collecting is its own command so a long run stays interruptible
        % and can be collected from another session. AutoCollect is for the
        % caller who wants one command to end with the reports in place.
        autoCollect = option_or_default( ...
            options.AutoCollect, cfg.PerCutAutoCollect);
        st_log(cfg, 'INFO', ...
            'PER_CUT run collected by the workflow | AutoCollect=%s', ...
            string(autoCollect));
        if autoCollect
...
elseif executeTests && any(~failedCloneRows)
    ...
    % possible in a later session.
    recordInfo = execute_timed_step('Save Run Record', ...
        @() st_save_run_record(runContext, workflowResult, plan, cfg));
