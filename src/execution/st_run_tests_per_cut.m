...
VerifyTimingStatus = repmat("NOT_RUN", n, 1);
ExpectedUpdateStatus = repmat("NOT_RUN", n, 1);
...
for i = 1:n
    ...
    try
        ...
        if row.ExpectedUpdateMode == "APPLY"
            ...
            ExpectedUpdateStatus(i) = string(updateGate.Status);
            ExpectedUpdatedCount(i) = updateGate.UpdatedCount;
            ...
            if ismember(updateGate.Status, {'PARTIAL', 'FAIL'})
                % A scenario that could not be updated is result data, not a
                % runner error. Keep the scenarios that were updated, rerun
                % them, and let the target judgment carry PARTIAL.
                st_log(cfg, 'WARN', ...
                    ['[PER_CUT %d/%d] expected update %s | TestCase=%s | ' ...
                     'ok=%d | fail=%d | updated lines=%d | %s'], ...
                    i, n, updateGate.Status, char(TestCaseName(i)), ...
                    updateGate.OkCount, updateGate.FailCount, ...
                    updateGate.UpdatedCount, updateGate.Message);
                append_event(logPath, i, 'EXPECTED_UPDATE_PARTIAL', ...
                    updateGate.Message);
            end
        end
        ...
        if ExpectedUpdatedCount(i) > 0 && cfg.RerunAfterExpectedUpdate
            ...
            VerifyTimingStatus(i) = string(st_combine_run_status( ...
        ...
        targetJudgment = string(st_combine_run_status( ...
            VerifyTimingStatus(i), ExpectedUpdateStatus(i)));
        if upper(FinalOutcome(i)) == "PASSED" && targetJudgment ~= "PARTIAL"
            Status(i) = "PASS";
            Message(i) = "Test Case completed and filter state restored";
        elseif upper(FinalOutcome(i)) == "PASSED"
            % Some scenarios of this CUT never produced usable verify timing
            % or could not be updated. Do not let the Test Manager verdict
            % hide that.
            Status(i) = "WARN";
            Message(i) = "Execution completed; verify timing=" + ...
                VerifyTimingStatus(i) + ", expected update=" + ...
                ExpectedUpdateStatus(i);
            st_log(cfg, 'WARN', ...
                ['[PER_CUT %d/%d] partial scenario coverage | TestCase=%s | ' ...
                 'verifyTiming=%s | expectedUpdate=%s'], ...
                i, n, char(TestCaseName(i)), ...
                char(VerifyTimingStatus(i)), char(ExpectedUpdateStatus(i)));
            append_event(logPath, i, 'TARGET_PARTIAL', char(Message(i)));
        else
...
targets = table(No, CUTName, CUTPath, ExecutionModel, StandaloneCUTPath, TestCaseName, ...
    ...
    RerunPerformed, ExpectedUpdatedCount, ...
    VerifyTimingStatus, ExpectedUpdateStatus, FilterApplyStatus, ...
...
function status = validate_verify_timing( ...
        resultObj, phase, targetRow, cfg, index, total, testCaseName, logPath)
% A CUT owns one Iteration per scenario. Aborting the whole Test Case because
% one scenario produced no verify result would also discard the expected-value
% update for the scenarios that ran, so only a total failure aborts.
verifyResult = st_validate_sldv_verify_results(resultObj, targetRow);
gate = st_verify_timing_gate(verifyResult, cfg, phase);
...
if strcmp(gate.Status, 'PARTIAL')
    st_log(cfg, 'WARN', ...
        ['[PER_CUT %d/%d] verify timing PARTIAL | TestCase=%s | phase=%s | ' ...
         'ok=%d | fail=%d | continuing | %s'], ...
        index, total, testCaseName, phase, gate.OkCount, gate.FailCount, ...
        gate.Message);
    append_event(logPath, index, 'VERIFY_TIMING_PARTIAL', gate.Message);
end
end
...
function manifest = build_target_manifest( ...
        ...
        verifyTimingStatus, expectedUpdateStatus, ...
...
manifest = struct( ...
    ...
    'VerifyTimingStatus', char(string(verifyTimingStatus)), ...
    'ExpectedUpdateStatus', char(string(expectedUpdateStatus)), ...
