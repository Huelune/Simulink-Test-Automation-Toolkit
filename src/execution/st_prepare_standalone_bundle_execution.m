...
metricSettings = lower(char(string(fileCoverage.MetricSettings)));
if ~contains(metricSettings, 'd')
    error('simtest:StandaloneCoverageMetricSettingsReadbackFailed', ...
        ['Standalone coverage metrics must include Decision. ' ...
         'Actual=%s'], metricSettings);
end
st_log(cfg, 'DEBUG', ...
