...
AvailableTestCaseCount = nan(n,1);
...
for i = 1:n
    ...
    % st_load_targets already blanked the cell for rows it does not apply
    % to (OFF, FILE+MAT) and normalized the rest.
    sldvTestCases = row_text(T(i,:), 'SldvTestCases');
    SldvTestCases(i) = string(sldvTestCases);
    ...
    profile.SldvTestCases = sldvTestCases;
    ...
    if ~selection.Run(i)
        try
            ...
            SldvTestCases(i) = string(profile_field( ...
                profile, 'SldvTestCases', ''));
            AvailableTestCaseCount(i) = profile_field( ...
                profile, 'AvailableTestCaseCount', NaN);
    ...
    try
        ...
        if strcmp(mode, 'OFF')
            ...
            profile.AvailableTestCaseCount = ...
                profile_field(meta, 'AvailableTestCaseCount', NaN);
            AvailableTestCaseCount(i) = profile.AvailableTestCaseCount;

            % A TestCase the operator asked for by number but SLDV marked as
            % having no effect is dropped like any other. Say so, because
            % the operator is counting on that number to appear.
            droppedSelected = profile_field( ...
                meta, 'DroppedSelectedIndices', []);
            if ~isempty(droppedSelected)
                st_log( ...
                    cfg, ...
                    'WARN', ...
                    ['[SLDV %d/%d] SldvTestCases named TestCase [%s] but ' ...
                     'SLDV marked it dataNoEffect, so no Scenario is made ' ...
                     'for it'], ...
                    i, ...
                    n, ...
                    char(strjoin(string(droppedSelected(:).'), ',')));
            end
            ...
            Status(i) = 'OK';
            Message(i) = sprintf( ...
                ['format=%s, scenarios=%d%s, RawTmax=%.17g, Tmax=%.17g, ' ...
                 'sldvInputs=%d, ' ...
                 'preservedHarnessInputs=%d, ignoredSldvInputs=%d'], ...
                dataFileFormat, ...
                ScenarioCount(i), ...
                describe_test_case_selection( ...
                    sldvTestCases, AvailableTestCaseCount(i)), ...
                meta.RawTmax, ...
                meta.Tmax, ...
...
% latest_sldvdata.mat keeps every generated TestCase. The SldvTestCases
% selection is applied when the file is read, so the same file can later be
% reused through SldvMode=FILE with a different selection.
try
    meta = inspect_sldv_data( ...
        candidateFile, ...
        ownerPath, ...
...
% by that position.
availableTestCaseCount = numel(data.TestCases);
[selectedIndices, selectionIsExplicit] = ...
    st_select_sldv_test_cases(sldvTestCases, availableTestCaseCount);
droppedSelectedIndices = zeros(0,1);

for sourceIndex = 1:numel(data.TestCases)
    tc = data.TestCases(sourceIndex);

    if ~ismember(sourceIndex, selectedIndices)
        continue;
    end

    if st_testcase_has_no_effect(tc)
        if selectionIsExplicit
            droppedSelectedIndices(end+1,1) = sourceIndex; %#ok<AGROW>
        end
        continue;
    end
...
if isempty(sourceIndices)
    if selectionIsExplicit
        error( ...
            ['SldvTestCases [%s] left no effective TestCase: every ' ...
             'selected TestCase is marked dataNoEffect. Available=%d'], ...
            char(strjoin(string(selectedIndices(:).'), ',')), ...
            availableTestCaseCount);
    end
    error( ...
...
meta = struct();
meta.AvailableTestCaseCount = availableTestCaseCount;
meta.DroppedSelectedIndices = droppedSelectedIndices;
...
function value = row_text(row, field)
%ROW_TEXT Read one optional text cell of a target row as char ('' if absent).
% Hand-built target tables in unit tests omit optional columns.
...
function text = describe_test_case_selection(sldvTestCases, availableCount)
%DESCRIBE_TEST_CASE_SELECTION Message fragment for the SldvTestCases column.

if isempty(sldvTestCases)
    text = '';
    return;
end
text = sprintf(' (SldvTestCases=%s of %g)', sldvTestCases, availableCount);
