...
coverageValid = strcmpi(cfg.CoverageStructuralLevel, 'Decision') && ...
    ...
    ismember(upper(string(cfg.CoverageFilterApplicationMode)), ...
        ["RUNTIME","PERSIST"]);
checks = [checks; st_verification_check( ...
...
try
    executionMode = st_resolve_execution_mode( ...
        cfg.ExecutionMode, T, cfg.SystemUnderTestMode);
...
exportFiles = {fullfile(root, 'resources', 'export_bundle', ...
    ...
    'st_export_result_set_report.m'), fullfile(root, 'src', 'reporting', ...
    'st_match_coverage_descriptors.m')};
checks = [checks; st_verification_check( ...
...
standaloneFiles = {fullfile(root, 'src', 'execution', ...
    'st_run_tests_from_exported_harnesses.m'), ...
    fullfile(root, 'src', 'coverage', ...
    'st_generate_standalone_coverage_filters.m'), ...
    fullfile(root, 'src', 'test_manager', ...
    'st_create_standalone_test_manager.m'), ...
    fullfile(root, 'diagnostics', 'matlab', ...
    'st_check_standalone_run.m')};
checks = [checks; st_verification_check( ...
    'CURRENT.STANDALONE_STATIC', 'STANDALONE_SUT', ...
    profile, target, true, ...
    pass_if(all(cellfun(@isfile, standaloneFiles))), ...
    'Standalone Harness model execution components are available')];
...
systemUnderTestMode = st_resolve_system_under_test_mode( ...
    cfg.SystemUnderTestMode);
executionMode = st_resolve_execution_mode( ...
    cfg.ExecutionMode, T, systemUnderTestMode);
if strcmp(systemUnderTestMode, 'EXPORTED_MODEL')
    pointer = cfg.StandaloneLatestPointer;
