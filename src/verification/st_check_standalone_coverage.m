...
tf = double(manifest.Version) == 2 && isfield(manifest, 'Actions') && ...
...
switch action
    ...
    case {"SUMMARY","ALL"}
        tf = action_ok(manifest, 'EXECUTE') && ...
            action_ok(manifest, 'PACKAGE') && ...
            action_ok(manifest, 'SUMMARY') && status == "OK";
    otherwise
        tf = false;
end
end
...
tf = status_ok(item, 'FilterRestoreStatus') && ...
    status_ok(item, 'ModelCleanupStatus') && ...
...
tf = counts.Model == numel(targets) && ...
    ...
    counts.CVF == numel(targets) && ...
    counts.CVT == numel(targets) && ...
    counts.HTML == numel(targets) && ...
