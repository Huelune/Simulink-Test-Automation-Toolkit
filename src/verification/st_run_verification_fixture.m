...
try
    [finalResult, updateResult, workflowResult, reportInfo] = ...
        ...
        'FromStage', 'START', 'ReportMode', 'FULL', ...
        'FailOnNonPass', false);
...
checks = [checks; inspect_fixture_specification( ...
    options, runDirectory)];
checks = [checks; inspect_fixture_results( ...
    updateResult, targets, options, reportInfo)];
...
try
    [~, ~, cachedWorkflow, cachedReport] = ...
        ...
        'ReportMode', 'FULL', 'FailOnNonPass', false);
...
function checks = inspect_fixture_specification(options, runDirectory)
checks = st_empty_verification_checks();
started = timestamp_text(); timerValue = tic;
output = [tempname(runDirectory) '.xlsx'];
try
    [specification, output] = st_export_test_specification( ...
        'OutputFile', output);
    targetRows = specification( ...
        string(specification{:,'대상 모델명'}) == "NoInportOff", :);
    expected = string(st_scenario_name('NoInportOff', 1));
    scenarioNames = string(targetRows{:,'InputScenario명'});
    inputFiles = string(targetRows{:,'하네스 input 파일명'});
    statuses = string(targetRows{:,'추출상태'});
    passed = height(targetRows) > 0 && ...
        all(scenarioNames == expected) && ...
        all(strlength(inputFiles) > 0 & inputFiles ~= "해당 없음") && ...
        all(statuses ~= "FAIL");
    checks = append(checks, row( ...
        'FIXTURE.NO_INPORT_SPECIFICATION', 'SPECIFICATION', options, ...
        pass_if(passed), ...
        ['No-Inport OFF target retains its Test Manager input scenario ' ...
         'and Signal Editor file in the exported specification'], ...
        output, toc(timerValue), started));
catch ME
    checks = append(checks, exception_row( ...
        'FIXTURE.NO_INPORT_SPECIFICATION', 'SPECIFICATION', options, ME, ...
        output, toc(timerValue), started));
end
end
...
sldvIndex = find(targets.SldvMode == "GENERATE", 1);
atomicTargetOk = ~isempty(sldvIndex);
if atomicTargetOk
    sldvOwner = st_normalize_cut_path( ...
        targets.CUTPath(sldvIndex), cfg.TopModel);
    atomicTargetOk = strcmp( ...
        get_param(sldvOwner, 'TreatAsAtomicUnit'), 'on');
end
checks = append(checks, row('FIXTURE.SLDV.ATOMIC_TARGET', 'SLDV', ...
    options, pass_if(atomicTargetOk), ...
    'SLDV target remains an Atomic Subsystem after preparation', ...
    cfg.ModelFile));
...
summary = reportInfo.Targets;
allPassed = ~isempty(summary) && ...
    all(upper(summary.FinalOutcome) == "PASSED");
checks = append(checks, row('FIXTURE.FINAL_PASS', 'EXECUTION', options, ...
    pass_if(allPassed), ...
    sprintf('%d/%d final Test Cases passed', ...
        sum(upper(summary.FinalOutcome) == "PASSED"), height(summary)), ...
    reportInfo.RunDirectory));

offRows = updateResult([],:);
if ~isempty(updateResult) && ismember('TargetNo', ...
        updateResult.Properties.VariableNames)
    offNo = targets.No(targets.ExpectedUpdateMode == "OFF");
    offRows = updateResult(ismember(updateResult.TargetNo, offNo),:);
end
...
try
    ...
    filterOk = sum(active) == 3 && ...
        ...
        all(filters.FilterRestoreStatus == "OK") && ...
        all(isfile(filters.CVFPath(active)));
    checks = append(checks, row('FIXTURE.COVERAGE.FILTER', 'COVERAGE', ...
        options, pass_if(filterOk), ...
        'Per-target coverage filter was generated and reported', ...
        reportInfo.Summary));
catch ME
...
htmlComplete = true;
for i = 1:height(targets)
    targetDirectory = fileparts(char(targets.TargetManifest(i)));
    required = [required, { ... %#ok<AGROW>
        char(targets.TargetManifest(i)), ...
        fullfile(targetDirectory, 'initial', 'TestSummary.xlsx'), ...
        fullfile(targetDirectory, 'initial', 'raw', 'InitialResults.mldatx'), ...
        fullfile(targetDirectory, 'initial', 'official', ...
            'InitialTestResults.pdf')}];
    initialHtml = dir(fullfile(targetDirectory, 'initial', ...
        'coverage', '*.html'));
    htmlComplete = htmlComplete && ~isempty(initialHtml);
    required = [required, fullfile({initialHtml.folder}, ...
        {initialHtml.name})]; %#ok<AGROW>
    if targets.RerunPerformed(i)
        required = [required, { ... %#ok<AGROW>
            fullfile(targetDirectory, 'final', 'TestSummary.xlsx'), ...
            fullfile(targetDirectory, 'final', 'raw', 'FinalResults.mldatx'), ...
            fullfile(targetDirectory, 'final', 'official', ...
                'FinalTestResults.pdf')}];
        finalHtml = dir(fullfile(targetDirectory, 'final', ...
            'coverage', '*.html'));
        htmlComplete = htmlComplete && ~isempty(finalHtml);
        required = [required, fullfile({finalHtml.folder}, ...
            {finalHtml.name})]; %#ok<AGROW>
    end
end
exists = cellfun(@isfile, required);
complete = all(exists);
complete = complete && htmlComplete;
checks = append(checks, row('FIXTURE.REPORT.ARTIFACTS', 'REPORTING', ...
    options, pass_if(complete), ...
    sprintf('%d/%d required per-CUT report files exist', ...
        sum(exists), numel(exists)), ...
    reportInfo.RunDirectory));
end
...
try
    ...
    owner = st_normalize_cut_path( ...
        targets.CUTPath(rowIndex), cfg.TopModel);
    set_param(owner, 'TreatAsAtomicUnit', 'off');
    ...
    refreshedProfiles = refreshed.manifest.Profiles;
    refreshedIndex = find(strcmp({refreshedProfiles.Mode}, 'FILE'), 1);
    fileMode = ~isempty(refreshedIndex) && ...
        strcmp(refreshedProfiles(refreshedIndex).AtomicAction, 'CONVERTED') && ...
        strcmp(get_param(owner, 'TreatAsAtomicUnit'), 'on');
    checks = append(checks, row('CERTIFY.FIXTURE.SLDV.FILE', 'SLDV', ...
        options, pass_if(fileMode), ...
        ['Generated SLDV data was reused through FILE mode after ' ...
         'persistent Atomic Subsystem conversion'], ...
        reportInfo.RunDirectory, toc(timerValue), started));
catch ME
    checks = append(checks, exception_row( ...
...
pointerBefore = st_file_signature(cfg.PerCutLatestPointer);
try
    targets = readtable(cfg.ManagementExcel, 'Sheet', cfg.ManagementSheet, ...
        'TextType', 'string', 'VariableNamingRule', 'preserve');
    ...
    pointerAfter = st_file_signature(cfg.PerCutLatestPointer);
...
try
    ...
    specificationFile = [tempname(cfg.ResultDir) '.xlsx'];
    [specification, specificationFile] = st_export_test_specification( ...
        'OutputFile', specificationFile);
    specStatus = string(specification{:,'추출상태'});
    specInputFile = string(specification{:,'하네스 input 파일명'});
    specInputContent = string(specification{:,'input 시나리오 내용'});
    specNote = string(specification{:,'비고'});
    if strcmp(variant, 'MISSING')
        passed = height(result) == 1 && height(specification) > 0 && ...
            string(result.Status(1)) == "SKIP_NO_SIGNAL_EDITOR" && ...
            all(specStatus == "WARN") && ...
            all(specInputFile == "해당 없음") && ...
            all(specInputContent == "해당 없음") && ...
            all(contains(specNote, "SKIP_NO_SIGNAL_EDITOR"));
        message = ...
            ['Missing Signal Editor returns WARN/SKIP_NO_SIGNAL_EDITOR ' ...
             'in configuration and specification'];
    else
        passed = height(result) == 1 && height(specification) > 0 && ...
            string(result.Status(1)) == "FAIL" && ...
            all(specStatus == "FAIL");
        message = ...
            'Broken Signal Editor MAT configuration fails both readers';
    end
