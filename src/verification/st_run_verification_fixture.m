...
try
    ...
    filters = readtable(reportInfo.Summary, 'Sheet', 'CoverageFilters', ...
        'TextType', 'string', 'VariableNamingRule', 'preserve');
    active = filters.FilterMode ~= "OFF";
    filterOk = sum(active) == 1 && ...
        all(filters.RuleCount(active) > 0) && ...
        all(filters.Status(active) == "OK") && ...
        all(isfile(filters.FilterFile(active)));
