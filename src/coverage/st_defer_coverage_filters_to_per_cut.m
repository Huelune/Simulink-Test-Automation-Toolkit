...
totalTimer = tic;
st_log(cfg, 'INFO', ...
    'Coverage filter deferral start | Targets=%d', height(T));
if nargin < 1
    stageSelection = [];
end
...
active = st_coverage_filter_active(T);
...
st_log(cfg, 'INFO', ...
    ['Coverage filter deferral complete | Active=%d | ' ...
     'elapsed=%.3f sec'], sum(active), toc(totalTimer));
end
