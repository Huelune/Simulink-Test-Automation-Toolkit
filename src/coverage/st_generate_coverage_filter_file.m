function result = st_generate_coverage_filter_file( ...
        targetRow, filterPath, cfg, varargin)
%ST_GENERATE_COVERAGE_FILTER_FILE Generate and verify one context-local CVF.

p = inputParser;
addParameter(p, 'ValidateSavedFilter', true, ...
    @(x) islogical(x) && isscalar(x));
parse(p, varargin{:});
validateSavedFilter = logical(p.Results.ValidateSavedFilter);
...
try
    ...
    if strcmp(boundaryMode, 'CUT_ONLY')
        ...
        for i = 1:numel(outside)
            ...
            [rulePaths, selectorTypes, filterModes, rationales, categories] = ...
                ...
                slcoverage.FilterMode.Exclude, 'none', ...
                'OUTSIDE_CUT');
        end
    end
    ...
    setFilterDescription(filterObj, 'none');
    ...
    serializationReadbackStatus = "NOT_REQUESTED";
    if validateSavedFilter
        validate_saved_filter(filterPath, ruleSids, selectorTypes, ...
            filterModes, rationales, rulePaths);
        serializationReadbackStatus = "OK";
    end
    result = struct( ...
        ...
        'SerializationReadbackStatus', serializationReadbackStatus, ...
        'Status', "OK", ...
        'Message', sprintf( ...
            'Generated %d CUT-child and %d boundary rule(s)', ...
