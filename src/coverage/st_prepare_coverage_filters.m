...
%
% Active content and boundary modes are combined in one verified CVF.
...
if any(st_coverage_filter_active(T)) && ~bdIsLoaded(cfg.TopModel)
    try
        st_log(cfg, 'DEBUG', ...
...
for i = 1:n
    ...
    active = st_coverage_filter_active(T(i,:));
    filterPath = st_coverage_filter_file(T(i,:), cfg);

    try
        if ~selection.Run(i)
            if active && isfile(filterPath)
        ...
        st_log(cfg, 'DEBUG', ...
            ['[CoverageFilter %d/%d] start | TestCase=%s | ' ...
             'Mode=%s | Boundary=%s | Action=%s'], ...
            i, n, char(T.TestCaseName(i)), char(mode), ...
            char(T.CoverageBoundaryMode(i)), ...
...
R = table(T.No, T.CUTName, T.CUTPath, T.TestCaseName, ...
    T.CoverageFilterMode, T.CoverageBoundaryMode, T.CoverageFilterAction, ...
    ...
    'FilterMode','BoundaryMode','FilterAction','Rationale','ApplicationMode', ...
