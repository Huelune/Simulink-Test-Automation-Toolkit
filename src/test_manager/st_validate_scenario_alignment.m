...
%
% OFF mode keeps the Iteration 1 display name, but its Signal Editor and
% Test Assessment parameters must both bind UT_REQ_{CUTName}_001. A Harness
% without a Signal Editor is valid and checks only Assessment/TC binding.
...
for i = 1:n
    ...
    try
        ...
        try
            ...
            signalEditorNames = st_normalize_options( ...
                get_param(sigBlock, 'options@ActiveScenario'));
            signalEditorNames = normalize_cellstr(signalEditorNames);
            activeScenario = char(get_param(sigBlock, 'ActiveScenario'));
            if isempty(activeScenario) || ...
                    ~strcmp(activeScenario, expected{1})
                error('simtest:SignalEditorActiveScenarioMismatch', ...
                    ['Signal Editor ActiveScenario does not match the ' ...
                     'first expected scenario. Expected=%s | Actual=%s'], ...
                    expected{1}, activeScenario);
            end
        catch findME
            if strcmp(profile.Mode, 'OFF') && ...
                ...
                st_log(cfg, 'WARN', ...
                    ['[ScenarioAlign %d/%d] Signal Editor check omitted | ' ...
        ...
        if SignalEditorAvailable(i)
            assert_same_scenario_set( ...
                expected, signalEditorNames, 'Signal Editor');
        end
        ...
        if strcmp(profile.Mode, 'OFF')
            ...
            expectedIterationNames = expected;
        end
        assert_same_scenario_set( ...
            expectedIterationNames, iterationNames, ...
...
function [signalEditorCount, testSequenceCount] = ...
        assert_iteration_scenario_parameters( ...
        iterations, expected, requireSignalEditor, offMode)
%ASSERT_ITERATION_SCENARIO_PARAMETERS Check both scenario consumers.
...
for i = 1:numel(iterations)
    ...
    if requireSignalEditor && ~test_param_matches_any_scenario( ...
            testParams, ...
