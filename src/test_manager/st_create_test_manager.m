...
if ~any(selection.Run)
    ...
    Action = repmat("CACHED", height(T), 1);
    Status = repmat("CACHED", height(T), 1);
    Message = PreparationReason;
    R = table(T.No, T.CUTName, T.TestCaseName, T.HarnessName, ...
        PreparationAction, PreparationReason, Action, Status, Message, ...
        'VariableNames', {'No', 'CUTName', 'TestCaseName', 'HarnessName', ...
            'PreparationAction', 'PreparationReason', 'Action', ...
            'Status', 'Message'});
...
covFile.RecordCoverage = ...
    true;
covFile.MetricSettings = ...
    cfg.CoverageMetricSettings;
covFile.MdlRefCoverage = ...
...
SignalEditorAvailable = false(n,1);
...
for i = 1:n
    ...
    try
        ...
        %% ====================================================
        % Direct CUT Inport (diagnostic only for scenario binding)
        %% ====================================================

        st_log(cfg, 'TRACE', ...
            '[TestManager %d/%d] searching direct CUT Inports', ...
            i, n);
        directInports = find_system(ownerPath, ...
            'SearchDepth', 1, 'Type', 'Block', 'BlockType', 'Inport');
        hasDirectInport = ~isempty(directInports);
        HasDirectInport(i) = hasDirectInport;
        ...
        if ~isempty(existingTc) && strcmp(profile.Mode, 'OFF') && ...
                (cfg.OverwriteTestFile || hasDirectInport)
        ...
        applySignalEditorScenario = false;
        if ~strcmp(profile.Mode, 'OFF')
        ...
        try
            signalBlock = st_find_signal_editor_block(harnessName);
            SignalEditorAvailable(i) = true;
            availableScenarios = st_normalize_options( ...
            ...
            if isempty(activeScenario) || ...
                    ~ismember(activeScenario, availableScenarios)
                error('simtest:SignalEditorActiveScenarioInvalid', ...
                    ...
                    activeScenario, strjoin(availableScenarios, ', '), ...
                    signalBlock);
            end
            missingScenarios = scenarioNames( ...
                ~ismember(scenarioNames, availableScenarios));
            if ~isempty(missingScenarios)
                error('simtest:SignalEditorScenarioMissing', ...
                    ['Required Signal Editor scenarios are missing. ' ...
                     'Missing=[%s] | Available=[%s]'], ...
                    strjoin(missingScenarios, ', '), ...
                    strjoin(availableScenarios, ', '));
            end
            st_collect_signal_editor_scenario_names( ...
                harnessName, cfg.TopModel);
        catch findME
            if strcmp(profile.Mode, 'OFF') && ...
                    strcmp(findME.identifier, ...
                    'simtest:SignalEditorBlockMissing')
                st_log(cfg, 'WARN', ...
                    ['[TestManager %d/%d] input scenario omitted | ' ...
                     'TestCase=%s | Harness=%s | ' ...
                     'reason=no Signal Editor block'], ...
                    i, n, testCaseName, harnessName);
            else
                rethrow(findME);
            end
        end
        ...
        elseif SignalEditorAvailable(i)
            ...
            Message(i) = ...
                ['Test Case + Iteration 1 + Assessment + Coverage created; ' ...
                 'input scenario omitted because Harness has no Signal Editor'];
        end


        fprintf( ...
            ['[%d/%d] OK   %s | ' ...
             'DirectInport=%d | SignalEditor=%d | ' ...
             'SignalEditorScenario=%d | Iterations=%d\n'], ...
            i, ...
            n, ...
            testCaseName, ...
            hasDirectInport, ...
            SignalEditorAvailable(i), ...
...
fprintf('UPDATED : %d\n', ...
    sum(Action == 'UPDATED_SLDV' | Action == 'UPDATED_INPUT'));
fprintf('FAILED  : %d\n', ...
    sum(Action == 'FAILED'));
fprintf('Total   : %d\n', ...
    n);
