...
try
    ...
    if logical(runContext.RerunPerformed)
        ...
        % target again would produce the same rows with a different label,
        % and that walk is the slowest part of this report.
        finalCoverage = initialCoverage;
        finalCoverage.Run(:) = "FINAL";
        st_log(cfg, 'INFO', ...
            ['Report step | Coverage FINAL | reused INITIAL ' ...
             '(no rerun, same ResultSet)']);
    end
